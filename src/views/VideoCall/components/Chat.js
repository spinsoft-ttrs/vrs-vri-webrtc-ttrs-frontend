...
const ResponsiveChatPush = (props) => {
  ...
  const renderText = (text) => {
    ...
    for (let i = 1; i < parts.length; i += 2) {
      parts[i] = (
        <a rel="noreferrer" target="_blank" key={"link" + i} href={parts[i]}>
          {parts[i]}
        </a>
      );
    }
...
const MessageRealTimeView = (props) => {
  if (props.messageRealtime !== "") {
    return (
      <div className="chat-row">
        <div className="chat-item-left" id="chat-item-left">
...
const Chat = (props) => {
  ...
  const handleSendLocation = () => {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      registerData.userAgent.sendMessage(
        `sip:${registerData.callNumber}@${registerData.domain}`,
        `ส่งพิกัดเรียบร้อย (${position.coords.latitude}, ${position.coords.longitude})`
      );
    });
  };
      ...
      <div className="shortcuts conversation">
        <div className="vrs_map" id="xxx">
          <div className="d-flex">
            <span className="txt-resize" onClick={(event) => increseTextSize()}>
              A+
            </span>
            <span className="txt-resize" onClick={(event) => decreseTextSize()}>
              A-
            </span>
          </div>
        </div>
        <div className="chat-content">
          <div className="chat-data" id="chat-data">
            {messagedata.map((chatdata, index) => {
              return <ResponsiveChatPush key={index} sender="" origin={chatdata.origin} date={chatdata.date} message={chatdata.body} />;
            })}
